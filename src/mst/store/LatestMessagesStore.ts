...
import { applySnapshot, destroy, IAnyType, types } from 'mobx-state-tree'
import {
    IMessage,
    IMessageSnapshotIn,
    IMessageSnapshotOut,
...
export const MessagesStore = types
    .model('MessagesStore', {
        ...
        users: types.array<IUserSnapshotOut>(User),
        messages: types.array<IMessageSnapshotOut>(Message),
    })
    .views((self) => ({
    ...
    .actions((self) => ({
        ...
        sortForReadAndUnread() {
            ...
            return self.messages.sort((a, b) => (a.isRead === b.isRead ? 0 : a.isRead ? 1 : -1))
        },
    }))
    .views((self) => ({
